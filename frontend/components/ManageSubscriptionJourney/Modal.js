...
class Modal extends Component {
    render() {
        const {day, subscription, overrideStartDate, overrideEndDate, quantity, amendFunctions, variant } = this.props
        ...
        return (
                    <ModalStyles>
                        <div className="modal__container">
                            ...
                            { variant === 'request' && 
                                <NumberInput value={quantity} onChange={amendFunctions.handleChange('quantity')}>
                                    {buildDropdown(subscription.product.availableStock, subscription.product.unit, true)}
                                </NumberInput> }
                            <DatesLayout>
                                <div>
                                    <label>Start Date</label>
                                    <DateInput onChange={amendFunctions.handleChange('overrideStartDate')} value={dateFns.format(overrideStartDate, 'YYYY-MM-DD')} type="date" placeholder="Starting date" />
                                </div>
                                <div>
                                    <label>End Date</label>
                                    <DateInput onChange={amendFunctions.handleChange('overrideEndDate')} value={dateFns.format(overrideEndDate, 'YYYY-MM-DD')} min={dateFns.format(overrideStartDate, 'YYYY-MM-DD')} type="date" placeholder="End date" />
                                </div>
                            </DatesLayout>
                            <ButtonStyles
                                onClick={() => this.props.amendFunctions.closeModal()}>
                                    Confirm
                            </ButtonStyles>
                        </div>
                    </ModalStyles>

...
class SubscriptionManagement extends Component {
  ...
  render() {
    ...
    return (
      <Query
        query={SINGLE_SUBSCRIPTION_QUERY}
        variables={{ id: this.props.enrollment }}
      >
        {({ error, loading, data }) => {
          ...
          return (
            <div>
              <h2>
                You are amending a subscription of{' '}
                {data.enrollment.product.name} from{' '}
                {data.enrollment.product.farm.name}
              </h2>
              <p>
                Please be mindful, if you are amending{' '}
                <strong>today's delivery</strong>, the{' '}
                <strong>cut off point is 2pm.</strong>
              </p>
              {(() => {
                switch (step) {
                  case 1:
                  ...
                  case 2:
                    return (
                      <AmendPage
                        ...
                        deliveryFrequency={
                          data.enrollment.product.deliveryFrequency
                        }

...
class SubscriptionDetails extends Component {
  ...
  render() {
    return (
      <>
        <Query
          query={SINGLE_PRODUCE_QUERY}
          variables={{ id: this.props.produceId }}
        >
          {({ error, loading, data }) => {
            ...
            this.props.subscription
              ? (nextDelivery = nextSubscriptionDay(
                  this.props.subscription.subscriptionFrequency,
                  this.props.subscription.subscriptionStartDate,
            ...
            return (
              <SubscriptionDetailsStyles>
                <div>
                  ...
                  <div>
                    <p>
                      Price: {formatMoney(product.price)} per {product.unit}
                    </p>
                    {this.props.isManage && this.props.subscription && (
                      <>
                        <p>
                          Your delivery quantity is{' '}
                          {this.props.subscription.quantity}{' '}
                          {this.props.subscription.quantity > 1
                            ? this.props.subscription.product.pluralUnit ||
        ...
        {this.props.isManage && (
          <Center>
            <ButtonStyles>
              <Link href={`/cancel?id=${this.props.subscription.id}`}>
                <a>Cancel Subscription</a>
              </Link>
            </ButtonStyles>

            <ButtonStyles onClick={this.amendSubscription}>
              Amend Subscription
            </ButtonStyles>
          </Center>
        )}

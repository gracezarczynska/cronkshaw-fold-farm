...
const Mutations = {
	async signup(parent, args, ctx, info) {
      ...
      <a href="${
			...
			}/activate-account?activationToken=${activationToken}">Click here to confirm your email address</a>`),
		});
	...
	async createEnrollment(parent, args, ctx, info) {
		...
		if (availableStock < 0) {
			throw new Error(
				`There is not enough stock for your purchase, there is ${
      ...
      \n\n
      Just to let you know, someone subscribed to your service from ${dateFns.format(
				args.subscriptionStartDate,
				'DD MM YYYY'
			)},\n
      ${args.subscriptionFrequency} and they fancy ${
				args.quantity
			} box(es) of eggs
      `),
		});
	...
	async updatePayment(parent, args, ctx, info) {
		const { userId } = ctx.request;
		if (!userId) {
			throw new Error('You must be signed in');
		}
		...
		try {
			...
			throw new Error(
				'Something went wrong, please contact us as soon as you can.'
			);
		}
	},
	...
	async createOverride(parent, args, ctx, info) {
        ...
        <a href="${
					process.env.FRONTEND_URL
				}/review?reviewToken=hello">Click here to review the request</a>`),
			});
			status = 'pending';
		}
	...
	async updateOverride(parent, args, ctx, info) {
		...
		const message = ctx.request.body.variables.message
			? `<br><p>Message from your farmer: ${
					ctx.request.body.variables.message
			  }</p>`
			: '';
		const mailRes = await transport.sendMail({
			from: 'dot@cronkshawfoldfarm.co.uk',
